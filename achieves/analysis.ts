...
export async function main(
	...
	let style: string = "";
	let sn: string = "";
	const default_db_key = DB_KEY_GACHA_URL_DEFAULT.replace( "$qq", user_id.toString() );
	if ( !raw_message ) {
		url = await redis.getString( default_db_key );
		if ( !url ) {
			await sendMessage( "缓存中无链接，请在命令后跟抽卡链接。" );
			return;
		}
		...
		try {
			...
			await redis.setString( default_db_key, url, DB_EXPIRE_24H );
		} catch ( error: any ) {
