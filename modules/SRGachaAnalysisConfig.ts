...
import { QiniuOssConfig } from "#sr_gacha_analysis/util/types";
import { checkDependencies, execHandle } from "#sr_gacha_analysis/util/util";
import bot from "ROOT";
import { PluginAlias } from "@modules/plugin";

export default class SRGachaAnalysisConfig {
	public static readonly FILE_NAME: string = "sr_gacha_analysis";
	private static init = {
	...
	public static create( file: FileManagement ): SRGachaAnalysisConfig {
		...
		if ( !isExist ) {
			...
			return new SRGachaAnalysisConfig( initCfg );
		}
		...
		if ( keysNum( config ) !== keysNum( initCfg ) ) {
			...
			return new SRGachaAnalysisConfig( c );
		}
		return new SRGachaAnalysisConfig( config );
	}
	...
	public async refresh( config ): Promise<string> {
		try {
			...
			for ( let uni_dep of uninstall_dependencies ) {
				bot.logger.info( `[星铁抽卡分析] 检测到 ${ uni_dep } 依赖尚未安装，将自动安装该依赖...` )
				const stdout = await execHandle( `npm i ${ uni_dep }` );
				bot.logger.info( stdout );
			}
			bot.logger.info( "[星铁抽卡分析] 所有插件需要的依赖已安装" );
			return "[星铁抽卡分析]插件配置重新加载完毕";
		} catch ( error ) {
			throw <RefreshCatch>{
				log: ( <Error>error ).stack,
				msg: "[星铁抽卡分析]插件配置重新加载失败，请前往控制台查看日志"
			};
		}
	}
}